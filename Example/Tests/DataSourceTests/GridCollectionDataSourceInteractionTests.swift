...
class GridCollectionDataSourceInteractionTests: XCTestCase {
    ...
    func testShouldDeselect() {
        ...
        let result = collection.delegate?.collectionView?(collection,
                                                          shouldDeselectItemAt: IndexPath(item: 1, section: 0))
        XCTAssert(result ?== true)
        XCTAssert(interactionMock.containerViewShouldDeselectCalled == true)
        XCTAssert(interactionMock.containerViewShouldDeselectCallsCount == 1)
    }
